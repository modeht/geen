...
export async function getDefs() {
  ...
  for (const file of files) {
    ...
    const fileImport = await import('./' + relativePathToFile).then(
      (m) => m.default,
    );

    let fileName = file.split('/').at(-1).replace('.schema.js', '');
    const defaultImport = fileImport?.default ? fileImport.default : fileImport;
    if (defaultImport) {
      fileName = fileName
        .split('-')
        .map((s) => s.charAt(0).toUpperCase() + s.slice(1))
        .join('');
      imports.push({ [fileName]: defaultImport });
      defs[fileName] = defaultImport;
    } else {
