...
export class BrandProfileController {
  ...
  constructor(private service: BrandProfileService){}
			...
			async create(
				@MoBody(CreateBrandProfileSchema) body: TCreateBrandProfileSchemaOutput,
			) {
			...
			async update(
				@Param('id') id: string,
				@MoBody(UpdateBrandProfileSchema) body: TUpdateBrandProfileSchemaOutput,
			) {
			...
			async read(
				@MoQuery(ReadBrandProfileSchema) query: TReadBrandProfileSchemaOutput,
			) {
