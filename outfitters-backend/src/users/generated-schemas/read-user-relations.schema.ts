...
import ReadAffiliationLinkTrackingRelationsSchema, {
...
const ReadUserRelationsSchema: v.GenericSchema<ReadUserRelations> = v.object({
	...
	affiliationLinkTracking: v.optional(
		v.union([
			...
			v.lazy(() => ReadAffiliationLinkTrackingRelationsSchema),
		]),
	),
	...
	receivedConversations: v.optional(
		v.union([
			v.pipe(
				v.union([v.string(), v.boolean()]),
				v.transform((input) => (input === 'true' ? true : false)),
	...
	savedCollections: v.optional(
		v.union([
			...
			v.lazy(() => ReadSavedCollectionRelationsSchema),
		]),
	),
	recentSearches: v.optional(
		v.union([
			v.pipe(
				v.union([v.string(), v.boolean()]),
				v.transform((input) => (input === 'true' ? true : false)),
			...
			v.lazy(() => ReadRecentSearchesRelationsSchema),
		]),
	),
	likedPosts: v.optional(
		v.union([
			v.pipe(
				v.union([v.string(), v.boolean()]),
				v.transform((input) => (input === 'true' ? true : false)),
