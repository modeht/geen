...
reviews: v.nullish(v.union([v.array(v.object({id:v.number()})), v.array(v.object({stars: v.nullish(v.number()),
comment: v.nullish(v.string()),
productId: v.number(),
shopperId: v.number()}))])),
addresses: v.nullish(v.union([v.array(v.object({id:v.number()})), v.array(v.object({deletedAt: v.pipe(v.string('Invalid type: Expected ISO timestamp string'), v.isoTimestamp()),
isDefault: v.boolean(),
name: v.string(),
country: v.string(),
city: v.string(),
...
longitude: v.string(),
shopperId: v.number()}))])),
profilePicture: v.nullish(v.union([v.object({ id: v.number() }), v.object({mimetype: v.nullish(v.string()),
...
engagementCount: v.nullish(v.number())}),v.metadata({[modelSymbol]: 'ShopperProfileEntity',
user: 'UserEntity',
