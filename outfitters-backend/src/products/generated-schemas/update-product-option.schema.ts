...
const UpdateProductOptionSchema = v.pipe(
	v.object({
		...
		values: v.nullish(
			v.union([
				...
				v.array(v.object({ value: v.string(), optionName: v.string(), productId: v.number() })),
			]),
		),
	}),
	v.metadata({ [modelSymbol]: 'ProductOptionEntity', product: 'ProductEntity', values: 'ProductOptionValueEntity' }),
);
