...
export const CreateProductVariantSchema = v.pipe(
	v.object({
		...
		optionValues: v.nullish(
			v.union([
				v.array(v.number()),
				v.array(
					v.object({ value: v.string(), optionName: v.string(), productId: v.number() }),
				),
			]),
		),
...
export type TCreateProductVariantSchema = v.InferInput<typeof CreateProductVariantSchema>;
