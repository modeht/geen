...
mainProduct: v.nullish(v.union([v.number(), v.object({isArchived: v.boolean(),
...
isSaved: v.boolean()})])),
optionValues: v.nullish(v.union([v.array(v.number()), v.array(v.object({value: v.string(),
optionName: v.string(),
productId: v.number()}))])),
mainProductId: v.nullish(v.number())}),v.metadata({media: 'MediaEntity',
