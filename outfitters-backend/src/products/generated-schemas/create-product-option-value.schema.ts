...
const CreateProductOptionValueSchema = v.pipe(
	v.object({
		value: v.string(),
		optionName: v.string(),
		productId: v.number(),
		option: v.nullish(v.union([v.object({ id: v.number() }), v.object({ name: v.string(), productId: v.number() })])),
		variants: v.nullish(
			v.union([
				v.array(
					v.object({
	...
	v.metadata({
		[modelSymbol]: 'ProductOptionValueEntity',
		option: 'ProductOptionEntity',
		variants: 'ProductVariantEntity',
	}),
);
export default CreateProductOptionValueSchema;
