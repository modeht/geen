...
} from './read-saved-collection-item-relations.schema';
import ReadUserRelationsSchema, { ReadUserRelations } from '../../users/generated-schemas/read-user-relations.schema';

export class ReadSavedCollectionRelations {
	items?: ReadSavedCollectionItemRelations | string | boolean;
	user?: ReadUserRelations | string | boolean;
}

const ReadSavedCollectionRelationsSchema: v.GenericSchema<ReadSavedCollectionRelations> = v.object({
	items: v.optional(
		v.union([
			v.pipe(
