...
export const UpdatePostLikesSchema = v.pipe(v.object({user: v.nullish(v.union([v.object({ id: v.number() }), v.object({status: v.enum(AccountStatus),
...
postId: v.optional(v.number())}),v.metadata({[modelSymbol]: 'PostLikesEntity',
user: 'UserEntity',
post: 'PostEntity'}))
