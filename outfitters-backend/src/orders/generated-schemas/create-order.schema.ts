...
const CreateOrderSchema = v.pipe(v.object({paymentMethod: v.enum(OrderPaymentMethod),
...
orderId: v.number(),
brandId: v.number()}))])),
shippingAddress: v.nullish(v.union([v.object({ id: v.number() }), v.object({deletedAt: v.pipe(v.string('Invalid type: Expected ISO timestamp string'), v.isoTimestamp()),
isDefault: v.boolean(),
name: v.string(),
country: v.string(),
city: v.string(),
...
shopperId: v.number()}),v.metadata({[modelSymbol]: 'OrderEntity',
cart: 'CartEntity',
