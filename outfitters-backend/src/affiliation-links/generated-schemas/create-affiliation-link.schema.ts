...
const CreateAffiliationLinkSchema = v.pipe(v.object({isDisabled: v.boolean(),
url: v.string(),
...
cartItems: v.nullish(v.union([v.array(v.object({id:v.number()})), v.array(v.object({quantity: v.nullish(v.number()),
cartId: v.number(),
productId: v.number(),
variantId: v.number(),
...
isSaved: v.boolean()})])),
productId: v.number(),
shopperId: v.number()}),v.metadata({[modelSymbol]: 'AffiliationLinkEntity',
