...
import {
	ReadTaggedProductRelationsSchema,
...
} from './read-affiliation-link-tracking-relations.schema';
import {
...
export class ReadAffiliationLinkRelations {
...
export const ReadAffiliationLinkRelationsSchema: v.GenericSchema<ReadAffiliationLinkRelations> =
	v.object({
		taggedProducts: v.undefinedable(
			v.union([
				v.pipe(
		...
		affiliationLinkTracking: v.undefinedable(
			v.union([
				v.pipe(
					v.union([v.string(), v.boolean()]),
					v.transform((input) => (input === 'true' ? true : false)),
		...
		shopperProfile: v.undefinedable(
			v.union([
				v.pipe(
					v.union([v.string(), v.boolean()]),
					v.transform((input) => (input === 'true' ? true : false)),
...
export type TReadAffiliationLinkRelationsSchemaOutput = v.InferOutput<
	typeof ReadAffiliationLinkRelationsSchema
>;
export type TReadAffiliationLinkRelationsSchemaInput = v.InferInput<
	typeof ReadAffiliationLinkRelationsSchema
>;
