...
export const UpdateMessageSchema = v.pipe(v.object({readAt: v.optional(v.pipe(v.string('Invalid type: Expected ISO timestamp string'), v.isoTimestamp())),
...
isSaved: v.boolean()})])),
conversation: v.optional(v.union([v.object({ id: v.number() }), v.object({isSupport: v.boolean(),
...
productId: v.nullish(v.number())}),v.metadata({[modelSymbol]: 'MessageEntity',
media: 'MediaEntity',
