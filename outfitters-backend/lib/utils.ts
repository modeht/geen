...
export class Utils {
	 ...
	 */
	static validateNullProperties(_obj: Record<string, any>, fields: string[]) {
		Object.entries(_obj).forEach(([key, val]) => {
			if (fields.includes(key)) {
				if (val === null) throw new BadRequestException(`Field ${key} cannot be null`);
	...
	static applyRelationsToQueryBuilder(alias: string, relations: any, queryBuilder) {
		const stack = [{ alias, relations }];

		while (stack.length > 0) {
			const { alias: currentAlias, relations: currentRelations } = stack.pop();

			Object.keys(currentRelations).forEach((key) => {
				const relationAlias = `${currentAlias}_${key}`;
				queryBuilder.leftJoinAndSelect(`${currentAlias}.${key}`, relationAlias);

				if (typeof currentRelations[key] === 'object' && currentRelations[key] !== null) {
					stack.push({
						alias: relationAlias,
						relations: currentRelations[key],
					});
				}
			});
		}
